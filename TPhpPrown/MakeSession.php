 ...
 * функция предлагает параметр $Init с указанием значения true.
 * 
 * Время жизни сессионной переменной, как правило, совпадает со временем, в 
 * течение которого у пользователя открыт браузер. Кроме этого для каждого 
 * сайта включается тайм-аут, по умолчанию равный 24 минутам. Если в течение 
 * этого времени никаких действий на сайте не происходит (а браузер продолжает 
 * быть открытым), то сессия завершается и умирают сессионные переменные. 
 * Это время может быть изменено на сервере в настройках PHP.
**/
