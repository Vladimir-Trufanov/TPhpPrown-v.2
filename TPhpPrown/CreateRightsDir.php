...
// пользователей. 
//
// Если каталог уже существует, то функция только выполняет попытку 
// переопределить права. В завершение работы функция сравнивает права, которые 
// установились, с заявленными правами, а именно с правами, которые должны были 
// установиться. Если желание не совпадает с фактом, то выдается сообщение об 
// ошибке или формируется исключение.
//
// Имя каталога задается по спецификации, то есть имя задается вместе с 
// относительным или абсолютным путем, например:
//
...
//
// Права определяются в традиционной нотации: https://www.php.net/manual/ru/function.chmod.php
// через трех-четырехразрядное восьмеричное число, задающее режим доступа к каталогу
// для владельца каталога, для группы, в которую входит владелец, и для прочих
// пользователей. 
// 
// Права передаются через биты соответствующих восьмеричных разрядов числа:
// http://f9r.ru/dostup.html
//
...
//
// Для полноценного просмотра каталога, необходимы права на чтение каталога и 
// доступ к файлам, а главное к их атрибутам, то есть минимальные разумные права
// на каталог - 5 (r-x). Прав 4 (r--) хватит только на просмотр имен файлов, 
// без атрибутов и не будут известны ни размер файла, ни права доступа.
// На практике для каталогов используется только три режима: 7 (rwx), 5 (r-x) и 
// 0 (---).
//
...
//
// Очень любопытный режим доступа к каталогу - 3 (-wx): он позволяет делать в 
// директории все, что угодно, но не позволяет прочитать имена объектов. То
// есть если вам не известны названия объектов в этом каталоге, то вы сделать с 
// ними ничего не сможете (даже удалить по маске * так как маску не к чему 
// применять - имена то недоступны.
//
// Ну и при работе с каталогами сервера, основанного на Unix, можно использовать 
// старший четвертый восьмеричный разряд, управляя битами SUID, SGID, Sticky. 
// https://help.ubuntu.ru/wiki/%D1%81%D1%82%D0%B0%D0%BD%D0%B4%D0%B0%D1%80%D1%82%D0%BD%D1%8B%D0%B5_%D0%BF%D1%80%D0%B0%D0%B2%D0%B0_unix#%D0%BF%D1%80%D0%B0%D0%B2%D0%B0_%D0%B4%D0%BE%D1%81%D1%82%D1%83%D0%BF%D0%B0_unix_suid_sgid_sticky_%D0%B1%D0%B8%D1%82%D1%8B
//
// SUID (Set User ID - бит смены идентификатора пользователя) и SGID (Set Group 
...
// 
// Установка Sticky-бита на каталог означает, что удалить файл из этого каталога 
// сможет только владелец файла или суперпользователь. Другие пользователи 
// лишаются права удалять файлы.
// 
// Это правило будет действовать и при вызове функции CreateRightsDir с установкой
// каждого бита "d rws rws rwt" следующим образом:
// 
// $ImgDir=$_SERVER['DOCUMENT_ROOT'].'/CreateRightsDir';
